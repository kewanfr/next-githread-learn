        ...
        <div className="flex -space-x-2">
          {user.followeds.map((f) => (
            <Avatar size="sm" className='border-2 border-background' key={f.follower.id}>
              {f.follower.image ? (
                <AvatarImage src={f.follower.image} alt={f.follower.username} />
              ) : null}
          ...
          {user.link ?(
            <>
            <p className="text-muted-foreground">{' · '}</p>
            <Link className='text-muted-foreground hover:underliner' href={user.link}>{removeHttp(user.link)}</Link>
            </>
          ): null}
      </div>
      {children}
